...
namespace RomM.Models.RomM.Rom
{
    ...
    public class RomMRom
    {
        ...
        // requested (?with_files=true). Call Normalize() after deserialize so consumers
        // never null-check; do not default here (tests + Normalize rely on null-when-absent).
        [JsonProperty("files")]
        public List<RomMFile> Files { get; set; }

        // RomM 4.9+/5.0 list schema uses sibling_roms (not "siblings").
