...
namespace RomM.Settings
{
    ...
    public class SettingsViewModel : ObservableObject, ISettings
    {
        ...
        public bool HasAnyAuth =>
            (!string.IsNullOrEmpty(RomMClientToken) && ApiTokenPattern.IsMatch(RomMClientToken)) ||
        ...
        // Stable RomM device identity for save sync (registered once). Separate from RomMDeviceID (QR login).
        ...
        internal SettingsViewModel(Plugin plugin, IRomM romM)
        {
            ...
            {  
                ...
                SkipMissingFiles = savedSettings.SkipMissingFiles;
        ...
        public bool TestConnection(bool UpdateNotificationBar = false, bool fetchProfile = true)
        {
            ...
            try
            {
                ...
                if (fetchProfile)
                {
                    ...
                    if (userinfo.Id > 0)
                    {
                        // RomM 5.0 removed /api/raw/assets — avatars are served from /api/users/{id}/avatar.
